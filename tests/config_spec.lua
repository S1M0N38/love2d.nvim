...
describe("config", function()
  ...
  describe("setup()", function()
    ...
    it("accepts lsp = false", function()
      config.setup({ lsp = false })
      assert.is_false(config.options.lsp)
    end)

    it("accepts lsp = true", function()
      config.setup({ lsp = true })
      assert.is_true(config.options.lsp)
    end)

    it("preserves lsp default when unspecified", function()
      config.setup({})
      assert.is_true(config.options.lsp)
    end)
  end)
